...
public static class TurTextFix
{
	...
	public static string Fix(string tur, string en)
	{
		...
		tur = Regex.Replace(tur, "\\s+,", ",");
		// LOTRO uses compact m/s/h suffixes for durations in skill and item
		// tooltips.  Normalize only translated text; source English remains
		// untouched and ms (milliseconds) is protected by the unit boundary.
		tur = DurationUnitFix.NormalizeTranslated(en, tur);
		tur = tur.Trim();
