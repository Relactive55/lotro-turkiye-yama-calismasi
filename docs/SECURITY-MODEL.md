...
- Translation pipeline `noop`/OPUS/Argos/Copilot/OpenAI-uyumlu provider
  sözleşmesine sahiptir; varsayılan private Actions akışı ücretsiz yerel Argos
  modelini kullanır ve kullanıcıdan veya API anahtarından ücret talep etmez.
  Copilot ya da özel API yalnız ayrıca yapılandırılırsa private secret/token
  alanında kullanılabilir; kaynak metni ve anahtar public çıktıya yazılmaz.
  Model revision/size/SHA-256 doğrulanmadan otomatik release'e bağlanmaz.
